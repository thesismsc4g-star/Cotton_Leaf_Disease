...
class ConvNeXtGCNImageEncoder(nn.Module):
    ...
    def __init__(
        ...
        with torch.no_grad():
            ...
            A_init = make_grid_adjacency_norm(
                self.h,
                self.w,
                device=device
            )
...
class ConvNeXtGCN_CLIP(nn.Module):
    ...
    def __init__(
        ...
        # PROMPT ATTENTION
        # ==================================
        self.prompt_attn_q = nn.Linear(
            self.text_embed_dim,
            self.text_embed_dim
        )

        self.prompt_attn_k = nn.Linear(
            self.text_embed_dim,
            self.text_embed_dim
        )

        self.prompt_attn_v = nn.Linear(
            self.text_embed_dim,
            self.text_embed_dim
        )
        ...
        self.num_prompts_per_class = []

        for cls in self.class_names:

            prompts = self.text_prompts[cls]

            self.num_prompts_per_class.append(
                len(prompts)
            )
    ...
    # TEXT EMBEDDINGS
    # ======================================
    def encode_prompt_embeddings(self):
        ...
        text_features = self.clip_model.text_projection(
    ...
    def encode_text_prototypes(self):
        ...
        for n in self.num_prompts_per_class:
            ...
            cls_prompts = prompt_embeds[
                start:start + n
            ]

            query = self.prompt_attn_q(
                cls_prompts.mean(
                    dim=0,
                    keepdim=True
                )
            ...
            keys = self.prompt_attn_k(
                cls_prompts
            )

            vals = self.prompt_attn_v(
                cls_prompts
            )
    ...
    def forward(self, x):
        ...
        image_embed, _, pooled, _, _ = self.image_encoder(
            x,
            return_nodes=True
        )
